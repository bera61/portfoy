...
function setVideos(vids) {
    setStorage('videos', vids);
}

function getProjects() {
    let projs = getStorage('projects', null);

    // If null or empty, seed defaults but ENSURE they all have an ID
...
function renderCategoryVideos(categoryId) {
    ...
    grid.innerHTML = videos.map((video, i) => {
        // We no longer have YouTube thumnails. If there is a custom thumbnail use it, else generic.
...
async function openVideoModal(videoId) {
    ...
    try {
        ...
        if (objectUrl) {
            container.innerHTML = `<video src="${objectUrl}" controls autoplay style="width:100%; height:100%; object-fit: contain; border-radius: var(--radius-lg);"></video>`;
        } else {
...
function initAdmin() {
    ...
    document.getElementById('settingsForm').addEventListener('submit', (e) => {
        ...
        document.getElementById('settingsSuccess').textContent = 'Ayarlar kaydedildi!';
        setTimeout(() => {
        ...
        adminProjList.innerHTML = projects.map(proj => {
            return `
                <div class="admin-list-item">
                    ...
                    <div class="admin-item-actions">
                        <button type="button" class="btn btn-danger btn-sm" onclick="deleteProject('${proj.id}')">Sil</button>
                    </div>
                </div>
            `;
        }).join('');
...
function deleteCategory(id) {
    if (!confirm('Bu kategoriyi ve altındaki tüm videoları silmek istediğinize emin misiniz?')) return;
